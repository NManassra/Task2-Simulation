...
*/
#define MAX_X 8  // Maximum rows for a 8x8 maze
#define MAX_Y 8  // Maximum columns for a 8x8 maze
...
void updateWalls() {
    int maxIndex = 7; // Maximum index for a 8x8 maze
...
void moveRobot() {
    ...
    if (facing == 1 && currCol < 7) { // Add boundary check for currCol
        currCol = currCol + 1;
        API_moveForward();
        return;
    }

    if (facing == 2 && currRow < 7) { // Add boundary check for currRow
        currRow = currRow + 1;
        API_moveForward();
        return;
    }
